				...
				var SekindoClientDynamicConfig = function (config, prob)
{
	...
	this.floatSetup = function(rules)
	{
		...
		if (typeof selectedRule[13] === 'undefined')
		{
			selectedRule[13] = 'right';
		}
		if (selectedRule[1] === 1)
		{
			ref.config.videoType = 'flow';
		...
		ref.config.floatConfig.closeBtnPos = selectedRule[13];
		...
		if (typeof selectedRule[14] !== 'undefined' && selectedRule[14].indexOf(ref.prob.geo) != -1 && selectedRule[14].indexOf('NO_DISALLOW_GEO') == -1)
		{
			ref.config.videoType = 'normal';
		}
...
var SekindoClientDetections_URL = function (config) {
	...
	this.setInfo = function()
	{
		...
		this.frameInfo.isBuildFrameViaJs = (typeof this.config.playerDivSetup.elmtClassName !== 'undefined' || typeof this.config.playerDivSetup.elmtId !== 'undefined' || typeof this.config.playerDivSetup.querySelector !== 'undefined') || (this.frameInfo.isBuildFrame && this.srcElement && (this.srcElement.async || this.srcElement.defer));
		if (this.debug)
		{
	...
	this.process = function()
	{
		...
		if (this.frameInfo.isBuildFrame)
		{
			...
			else
			{
				...
				{
					var code = '<div><div><iframe style="width:1px; height:1px;"  marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no" onload="construct'+uniqueID+'(this)" title ="Primis Frame" id ="google_ads_iframe_dummy_'+uniqueID+'"></iframe></div></div>';
				}
	...
	this.setInfo();
				...
				var urlDetObj = new SekindoClientDetections_URL({
					url: 'https://live.sekindo.com/live/liveView.php?s=104300&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]&cbuster=1619118634',
					origQString: 's=104300&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]',
					placementId: 104300,
					x: 600,
					y: 340,
					videoType: 'flow',
					...
					floatConfig: {
						...
						direction: 'br', verticalOffset: 1,
						horizontalOffset: 1, isCloseBtn: 1,
						flowMode: 'both', closeBtnPos: 'right'
					},
					isAPI: false,
					debug: 0,
